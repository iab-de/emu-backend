/*
 * Backend "Elektronischer Münzwurf"
 * Copyright (C) 2025 Institut für Arbeitsmarkt und Berufsforschung <https://www.iab.de>
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */

package de.arbeitsagentur.iab.emu.service.projekt;

import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertNotNull;
import static org.junit.jupiter.api.Assertions.assertNull;

class ProjektTest {

    @Test
    void validiereFelddefinitionen() {

        Projekt p = ProjektFactory.gueltigesProjektErzeugen();
        assertNull(p.validiereFelddefinitionen());
    }

    @Test
    void validiereFelddefinitionenMehrfacheBezeichnung() {

        Projekt p = ProjektFactory.gueltigesProjektErzeugen();
        p.getFelddefinitionen().forEach(f -> f.setName(null));
        assertNotNull(p.validiereFelddefinitionen());
    }

    @Test
    void validiereGruppen() {

        Projekt p = ProjektFactory.gueltigesProjektErzeugen();
        assertNull(p.validiereGruppen());
    }


    @Test
    void validiereGruppenGrenzueberschneidung() {

        Projekt p = ProjektFactory.gueltigesProjektErzeugen();
        // Ober- und Untergrenzen von zwei Gruppen überschneiden lassen:
        p.getGruppen().get(1).setUntergrenze(p.getGruppen().get(0).getObergrenze());

        assertNotNull(p.validiereGruppen());
    }

    @Test
    void validiereGruppenGrenzenUngueltig() {

        Projekt p = ProjektFactory.gueltigesProjektErzeugen();
        // Obergrenze = Untergrenze
        p.getGruppen().get(1).setUntergrenze(p.getGruppen().get(1).getObergrenze());

        assertNotNull(p.validiereGruppen());

    }

    @Test
    void validiereGruppenBezeichnungMehrfach() {

        Projekt p = ProjektFactory.gueltigesProjektErzeugen();

        p.getGruppen().get(1).setBezeichnung(p.getGruppen().get(0).getBezeichnung());

        assertNotNull(p.validiereGruppen());
    }

    @Test
    void validiereGruppenNurEineGruppe() {

        Projekt p = ProjektFactory.gueltigesProjektErzeugen();

        p.getGruppen().remove(1);

        assertNotNull(p.validiereGruppen());

    }

    @Test
    void validiereGruppenKeineGruppe() {

        Projekt p = ProjektFactory.gueltigesProjektErzeugen();

        p.getGruppen().clear();

        assertNotNull(p.validiereGruppen());

    }
}